...
// JavaScript (dabovl.js)

// Обработчик нажатия на кнопку "Добавить трек"
document.getElementById("addTrackButton").addEventListener("click", function() {
    ...
    formData.append("trackName", trackNameInput.value);
    formData.append("artistName", artistNameInput.value);

    // Создание AJAX-запроса
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "add_track.php", true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            // Обработка ответа от сервера
            var response = xhr.responseText;
            console.log(response);  // Можете использовать для отладки
            // Дополнительные действия по обновлению страницы или отображению сообщения об успешном добавлении трека
        }
    };
    xhr.send(formData);
});
